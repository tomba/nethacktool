...
{
	public partial class ScrollForm : UserControl
	{
		ListBox m_listBox;
		...
		public ScrollForm()
		{
			...
			scrollListView.Items.Clear();

			foreach (string color in ScrollContainer.ScrollTexts)
			{
				...
				scrollListView.Items.Add(item);
			}

			m_listBox = new ListBox();
			m_listBox.Parent = scrollListView;
			m_listBox.Hide();
			...
			scrollListView.Columns[0].Width = -1;
			scrollListView.Columns[3].Width = -2;
		...
		public void Clear()
		{
			foreach (ListViewItem item in scrollListView.Items)
			{
		...
		{
			doc.scrolls = new Document.Scroll[scrollListView.Items.Count];
			for (int i = 0; i < scrollListView.Items.Count; i++)
			{
				ListViewItem item = scrollListView.Items[i];
				doc.scrolls[i] = new Document.Scroll();
				doc.scrolls[i].text = item.Text;
				doc.scrolls[i].spell = item.SubItems["spell"].Text;
				doc.scrolls[i].price = item.SubItems["price"].Text;
			}
		}
		...
		public void Restore(Document doc)
		{
			...
			for (int i = 0; i < doc.scrolls.Length; i++)
			{
				Document.Scroll save = doc.scrolls[i];
				ListViewItem item = scrollListView.FindItemWithText(save.text);
		...
		void UpdateAlternatives()
		{
			...
			foreach (Scroll p in ScrollContainer.Scrolls)
				spells.Add(p.Spell);
			...
			foreach (ListViewItem item in scrollListView.Items)
			{
				...
				{
					if (ScrollContainer.Scrolls[spell].Price.ToString() == item.SubItems["price"].Text)
		...
		private void scrollListView_MouseUp(object sender, MouseEventArgs e)
		{
			ListViewItem item = scrollListView.GetItemAt(e.X, e.Y);
			...
			{
				m_listBox.Items.Add("");
				m_listBox.Items.AddRange(GetAvailableScrolls(subItem.Text));
				m_listBox.SelectedItem = subItem.Text;
			}
			else if (subItem.Name == "price")
			{
				m_listBox.Items.Add("");
				m_listBox.Items.AddRange(ScrollContainer.ScrollPrices);
				m_listBox.SelectedItem = subItem.Text;
			}
			else
			{
		...
		string[] GetAvailableScrolls(string skipSpell)
		{
			List<string> spells = new List<string>();
			foreach (Scroll p in ScrollContainer.Scrolls)
				spells.Add(p.Spell);
